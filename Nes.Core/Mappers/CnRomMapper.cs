...
internal sealed class CnRomMapper(Emulator emulator) : Mapper(emulator)
{
    ...
    public override byte ReadByte(ushort address)
    {
        ...
        {
            < 0x2000 => m_emulator.InstalledCartridge?.ChrData[m_ChrBank + address] ?? default,
    ...
    public override void WriteByte(ushort address, byte value)
    {
        ...
        {
            case < 0x2000:
                m_emulator.InstalledCartridge!.ChrData[m_ChrBank + address] = value;
                break;

            case >= 0x8000:
                m_ChrBank = value * 0x2000;
                m_ChrBank %= m_emulator.InstalledCartridge!.ChrRomSize;
                break;
        }
    }
}
